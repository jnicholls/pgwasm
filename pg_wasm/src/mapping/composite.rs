...
mod tests {
    ...
    fn pg_dynamic_marshal_round_trips() {
        ...
        Spi::run("DROP TYPE IF EXISTS wasm.marshal_pg_row CASCADE").unwrap();
        Spi::run("CREATE TYPE wasm.marshal_pg_row AS (a int4, b text)").unwrap();
        let row_oid: pg_sys::Oid = Spi::get_one("SELECT 'wasm.marshal_pg_row'::regtype::oid")
            .unwrap()
            .unwrap();
        ...
        Spi::run("DROP TYPE IF EXISTS wasm.marshal_pg_var CASCADE").unwrap();
        Spi::run("CREATE TYPE wasm.marshal_pg_var AS (disc text, payload int4)").unwrap();
        let var_oid: pg_sys::Oid = Spi::get_one("SELECT 'wasm.marshal_pg_var'::regtype::oid")
            .unwrap()
            .unwrap();
