...
/// `pg_wasm.reload(module_name, bytes_or_path, options)` — see architecture doc "Reload lifecycle".
#[pg_extern]
pub fn reload(
    module_name: &str,
...
fn require_loader_or_superuser() -> Result<()> {
    ...
    if allowed {
        ...
        Err(PgWasmError::PermissionDenied(
            "pg_wasm.reload requires superuser or membership in role `wasm_loader`".to_string(),
        ))
    }
}
