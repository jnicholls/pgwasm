//! Error types and conversion helpers used across the pgwasm extension.
    ...
    pub(crate) fn hint(&self) -> Option<&'static str> {
        match self {
            Self::PermissionDenied(_) => Some(
                "Check extension GUCs (e.g. pgwasm.allow_load_from_file, pgwasm.allow_spi, pgwasm.allow_wasi*) and module policy JSON; overrides may only narrow defaults.",
            ),
            _ => None,
        }
    }
