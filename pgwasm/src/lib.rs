...
mod tests {
    ...
    fn host_query_disabled_per_invocation_when_catalog_narrows_spi() {
        ...
        let (out,) = run.call(&mut store, ()).expect("run");
        assert!(
            out.starts_with("ERR:") && out.contains("pgwasm.allow_spi"),
    ...
    fn unload_udt_respects_dependencies_and_cascade() {
        ...
        let type_name = format!("pgwasm.tunload_T_{pid}");
    ...
    fn load_transaction_rollback_removes_artifact_dir() {
        ...
        unsafe {
            pg_sys::BeginInternalSubTransaction(c"pgwasm_load_rb_test".as_ptr());
        }
...
// Registered in the extension schema (`pgwasm` per pgwasm.control); SQL calls `pgwasm.pgwasm_core_invoke_scalar`.
