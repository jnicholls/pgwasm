    ...
    fn instantiate(
        ...
        let instance = linker
            ...
            .map_err(|error| {
                ...
                if message.contains("pgwasm:host/query") && message.contains("unknown import") {
                    PgWasmError::PermissionDenied(format!(
                        "component imports pgwasm:host/query but SPI is disabled; enable pgwasm.allow_spi ({message})"
                    ))
                } else {
