...
/// `pgwasm.pgwasm_load(module_name, bytes_or_path, options)` — see architecture doc "Load lifecycle".
pub(crate) fn load_impl(
    ...
    if let Some(existing) = modules::get_by_name(module_name)? {
        if opts.replace_exports {
            return Err(PgWasmError::ModuleAlreadyLoaded {
                ...
                msg: "module already loaded; use `pgwasm.pgwasm_reload(...)` to replace exports in place"
                    .to_string(),
            });
        }
        return Err(PgWasmError::ModuleAlreadyLoaded {
            module_id: existing.module_id,
            msg: format!(
                "module `{module_name}` already exists in catalog; use `pgwasm.pgwasm_reload(...)` or unload first"
            ),
        });
    }
    ...
    if max_module_bytes == 0 {
        if !bytes.is_empty() {
            return Err(PgWasmError::ResourceLimitExceeded(
                "module bytes are non-empty but pgwasm.max_module_bytes is 0".to_string(),
            ));
        }
        ...
        > max_u64
    {
        return Err(PgWasmError::ResourceLimitExceeded(format!(
            "module size {} bytes exceeds pgwasm.max_module_bytes ({max_u64})",
            bytes.len()
        )));
    }
    abi::validate(&bytes)?;
...
fn try_prewarm_component_pool_note(module_id_u64: u64) {
    ereport!(
        ...
        format!(
            "pgwasm: instance pool warm-up for module_id {module_id_u64} deferred until first call (prewarm API not available)"
        ),
    );
}
...
fn require_loader_or_superuser() -> Result<()> {
    ...
    if allowed {
        ...
        Err(PgWasmError::PermissionDenied(
            "pgwasm.pgwasm_load requires superuser or membership in role `pgwasm_loader`"
                .to_string(),
        ))
    }
}
