//! Unload lifecycle: catalog teardown, optional cascade over `pgwasm.dependencies`, and post-commit
...
pub(crate) fn unload_impl(module_name: &str, cascade: bool) -> Result<bool> {
    ...
    if other_modules_depend_on(module_id)? && !cascade {
        return Err(PgWasmError::InvalidConfiguration(format!(
            "module `{module_name}` is listed as a dependency of another module in `{EXTENSION_SCHEMA}.dependencies`; retry with `cascade := true` (hint: `cascade := true`)"
        )));
    }
...
fn require_loader_or_superuser() -> Result<()> {
    ...
    if allowed {
        ...
        Err(PgWasmError::PermissionDenied(
            "pgwasm.pgwasm_unload requires superuser or membership in role `pgwasm_loader`"
                .to_string(),
        ))
    }
}
...
fn require_superuser() -> Result<()> {
    ...
    if is_super {
        ...
        Err(PgWasmError::PermissionDenied(
            "pgwasm.pgwasm_unload_all requires superuser".to_string(),
        ))
    }
}
...
fn other_modules_depend_on(unloaded_module_id: i64) -> Result<bool> {
    let sql = format!(
        "SELECT EXISTS (
            SELECT 1
            FROM {EXTENSION_SCHEMA}.dependencies d
    ...
    let active = match active_module_ids() {
        ...
        Err(e) => {
            ereport!(
                ...
                format!("pgwasm unload: active_module_ids failed: {e}"),
            );
            BTreeSet::new()
        }
    };
...
fn post_commit_unload_work_inner(module_id_u64: u64, active_ids: &BTreeSet<u64>) {
    if let Err(e) = pool::drain(module_id_u64) {
        ereport!(
            PgLogLevel::WARNING,
            PgSqlErrorCode::ERRCODE_WARNING,
            format!("pgwasm unload: pool::drain failed for module_id {module_id_u64}: {e}"),
        );
    }
    ...
    if let Err(e) = artifacts::with_artifact_fs_lock(|| {
        ...
        ereport!(
            PgLogLevel::WARNING,
            PgSqlErrorCode::ERRCODE_WARNING,
            format!("pgwasm unload: artifact cleanup failed: {e}"),
        );
    }
...
pub(crate) mod test_support {
    ...
    pub fn count_modules_where_name(name: &str) -> i64 {
        Spi::get_one::<i64>(&format!(
            "SELECT count(*)::bigint FROM {}.modules WHERE name = '{}'",
            EXTENSION_SCHEMA,
            name.replace('\'', "''")
        ))
    ...
    pub fn count_exports_for_module(module_id: i64) -> i64 {
        Spi::get_one::<i64>(&format!(
            "SELECT count(*)::bigint FROM {}.exports WHERE module_id = {module_id}",
            EXTENSION_SCHEMA
        ))
    ...
    pub fn insert_dependency(module_id: i64, depends_on: i64) {
        Spi::run(&format!(
            "INSERT INTO {}.dependencies (module_id, depends_on_module_id) VALUES ({module_id}, {depends_on})",
            EXTENSION_SCHEMA
        ))
