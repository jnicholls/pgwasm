...
#[doc(hidden)]
#[unsafe(no_mangle)]
pub extern "C" fn pg_finfo_pgwasm_udf_trampoline() -> &'static Pg_finfo_record {
...
fn trampoline_inner(fcinfo: FunctionCallInfo) -> Result<Datum, TrampolineReport> {
    ...
    if fn_oid == Oid::INVALID {
        return Err(
            PgWasmError::Internal("pgwasm_udf_trampoline: missing fn_oid".to_string()).into(),
        );
    }
    ...
    Ok(match oid {
        ...
        _ => {
            return Err(PgWasmError::Unsupported(format!(
                "pgwasm trampoline: unsupported argument type oid {}",
                u32::from(oid)
            )));
        }
    })
...
mod tests {
    ...
    fn install_component_module_row(
        ...
        let stub_row = modules::insert(&modules::NewModule {
            ...
            artifact_path: "/tmp/pgwasm_trampoline_stub".to_string(),
            digest: digest.clone(),
            generation: 0,
            limits: limits.clone(),
            name: name.clone(),
    ...
    fn trampoline_component_returns_and_increments_counters() {
        ...
        Spi::run("SET pgwasm.fuel_enabled = on").unwrap();
