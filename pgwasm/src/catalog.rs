...
pub(crate) mod migrations {
    ...
    fn extension_is_installed() -> core::result::Result<bool, PgWasmError> {
        Spi::get_one::<bool>(
            "SELECT EXISTS (SELECT 1 FROM pg_catalog.pg_extension WHERE extname = 'pgwasm')",
        )
...
mod tests {
    ...
    fn test_catalog_shape_and_role_grants() {
        ...
        assert!(role_exists("pgwasm_reader"));
        assert!(role_exists("pgwasm_loader"));
        assert!(has_schema_privilege("pgwasm_reader", "USAGE"));
        assert!(has_schema_privilege("pgwasm_loader", "USAGE"));
        ...
        for (table_name, _) in EXPECTED_TABLE_COLUMNS {
            ...
            for privilege in ["SELECT", "INSERT", "UPDATE", "DELETE"] {
                assert!(
                    has_table_privilege("pgwasm_loader", qualified_table_name.as_str(), privilege),
                    "expected pgwasm_loader to have {privilege} on {qualified_table_name}"
                );
            }
        }
    }
    ...
    fn has_schema_privilege(role_name: &str, privilege: &str) -> bool {
        Spi::get_one_with_args(
            "SELECT pg_catalog.has_schema_privilege($1, 'pgwasm', $2)",
