...
-- Seed a minimal module + export (superuser bypasses pgwasm_loader for INSERT).
INSERT INTO pgwasm.modules (
    module_id,
    name,
    abi,
    digest,
...
FROM pgwasm.pgwasm_policy_effective()
WHERE module_name = 'views_test_mod'
ORDER BY module_name;

-- Allocate shmem slots and bump invocations (superuser-only helper).
SELECT pgwasm.pgwasm_test_bump_export_counters(m.module_id, 0, 3) AS bumped
FROM pgwasm.modules m
WHERE m.name = 'views_test_mod';
...
SELECT pgwasm.pgwasm_test_bump_export_counters(m.module_id, 0, 2) AS bumped_again
FROM pgwasm.modules m
WHERE m.name = 'views_test_mod';
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'views_test_reader') THEN
        CREATE ROLE views_test_reader NOLOGIN IN ROLE pgwasm_reader;
    END IF;
END
$reader_block$;
...
SELECT COUNT(*) AS n_modules FROM pgwasm.modules_view;
SELECT COUNT(*) AS n_functions FROM pgwasm.functions_view;
SELECT COUNT(*) AS n_wit FROM pgwasm.wit_types_view;
SELECT COUNT(*) AS n_policy FROM pgwasm.policy_effective_view;
SELECT COUNT(*) AS n_stats FROM pgwasm.stats_view;

RESET ROLE;

DELETE FROM pgwasm.exports WHERE module_id = (SELECT module_id FROM pgwasm.modules WHERE name = 'views_test_mod');
DELETE FROM pgwasm.modules WHERE name = 'views_test_mod';
